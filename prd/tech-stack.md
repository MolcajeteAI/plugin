...
All entity IDs use `PREFIX-{tag}` where `{tag}` is a 4-character base-62 code generated by the shared script.

- Features: `FEAT-{tag}`
- Use Cases: `UC-{tag}`
- Scenarios: `SC-{tag}`
- Functional Requirements: `FR-{tag}`
- Non-Functional Requirements: `NFR-{tag}`
- User Stories: `US-{tag}`
- Architecture Decision Records: `ADR-{tag}`
