...
{Repeat for each module/application. Omit fields that don't apply (e.g., no Styling for a backend service). For single-app projects, use one module section. The **Running tests** and **Coverage** fields are required for every module that ships testable code — `/m:execute` reads them to run the TDD loop and the 80% coverage gate.}
