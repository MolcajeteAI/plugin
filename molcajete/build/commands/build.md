...
- `prd/PROJECT.md`
- `prd/MODULES.md`
- UC file: Glob `prd/modules/*/features/*/use-cases/{task.use_case}-*.md` to find and read it
- Feature REQUIREMENTS.md: derive from the UC path (same feature directory)
- `.molcajete/settings.json` (if exists) -- for BDD framework/language settings

**Read if path is non-empty:**
- Architecture: `{task.architecture}`
- `prd/TECH-STACK.md`

**Read if Gherkin exists:**
...
- `bdd/steps/INDEX.md` (if exists)

**Extract from plan.md:**
- Locate the `### T-NNN` section in `plan.md`. If the task ID has a sub-task format (`T-NNN-M`), use the parent ID (`T-NNN`) to find the section.

**Collect prior summaries:**
- For each task ID in `task.depends_on`, read its `summary` field from `plan.json`. These provide context about what was already built.
