...
**The UC is the leaf.** There are no slice files. The finest-grained unit of build work is a **task** — a `## [ ] T-NNN` checkbox in a plan (`specs/plans/<plan-id>.md`). Task state is the checkbox itself (`[ ]` pending, `[x]` done), not a frontmatter `status`. `/m:build` reads task checkboxes to write UC status directly, and Feature status rolls up from its UCs.
