---
description: Headless task implementation (used by dispatch.sh, not user-facing)
model: claude-opus-4-6
allowed-tools: Read, Write, Edit, Glob, Grep, Bash(*), Task
argument-hint: <task ID like "UC-0Fy0-001/1.1" or feature like "Feature 2">
---

# Implement Task (Headless Mode)
...
3. If dependencies are NOT completed, log `[HEADLESS] WARNING: Proceeding despite incomplete dependencies: {list}` and continue.
...
If anything about the task is ambiguous after reading the context brief, make your best judgment based on the available context. Document any assumptions or decisions in the plan file (Step 3) under a "Headless Decisions" section.

If the task is clear, skip this step.

## Execution Scope

This command executes exactly ONE sub-task (e.g., "UC-0Fy0-001/1.1"). The external coordinator manages iteration across sub-tasks. If `$ARGUMENTS` points to a parent task (e.g., "UC-0Fy0-001/1"), implement the first incomplete sub-task and log: `[HEADLESS] Parent task specified. Selected first incomplete sub-task: {sub-task ID}`.
...
- `EXIT != 0` (tests fail): Send failure output to Developer agent for fixes. Max 3 cycles, then stop and report failure.
...
3. If Reviewer found CRITICAL or WARNING issues:
   ...
   d. Max 3 fix cycles. If the review still has CRITICAL issues after 3 cycles, output failure:
      ```json
...
2. If COMMIT_STATUS is HOOK_FAILURE:
   ...
   c. Max 3 hook-failure cycles. If commit still fails after 3 cycles, output failure:
      ```json
...
- Never block on user input. At every decision point, make the best autonomous decision and log the reasoning.
