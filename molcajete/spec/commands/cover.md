---
description: Extract specs from existing code (scope inferred from freeform input). Tests come later via molcajete build.
model: claude-opus-5
...
**Authoring only.** `/m:cover` writes spec prose, populates ARCHITECTURE.md and the module contract files, appends a log entry, and writes a cover request. It does **not** produce plans, code, or tests.

`/m:cover` is the one authoring command that writes the spec tree directly — it documents shipped behavior, which changes no contract. Its `request.md` under `specs/changes/{change-id}/` is the list of the files to spec and test, so `molcajete build {change-id}` knows its scope.
...
5. `${CLAUDE_PLUGIN_ROOT}/spec/skills/module-authoring/SKILL.md` — the INTERFACE.md and DATA.md formats Step 7 populates from code.
6. `${CLAUDE_PLUGIN_ROOT}/spec/skills/change-request/SKILL.md` — the change directory and the cover-request shape Step 8 writes.
7. `${CLAUDE_PLUGIN_ROOT}/shared/skills/resolution-gate/SKILL.md` — the single gate that runs when the discovery scan finishes and before Step 7 writes anything.
8. `${CLAUDE_PLUGIN_ROOT}/shared/skills/id-generation/SKILL.md`
9. `${CLAUDE_PLUGIN_ROOT}/shared/skills/uc-log/SKILL.md` — CHANGELOG mechanics only.
10. `${CLAUDE_PLUGIN_ROOT}/shared/skills/status-rollup/SKILL.md` — how to write UC and Feature status directly.
...
Write the spec content for the resolved scope: REQUIREMENTS.md, UC spec files (`UC-XXXX-{slug}.md` as siblings of REQUIREMENTS.md / USE-CASES.md / ARCHITECTURE.md), UC support folders (each containing an initialized `CHANGELOG.md` with empty TODO/DONE sections), and appended scenarios as appropriate. Every ID-bearing heading gets its explicit `<a id="{ID}"></a>` anchor line, per the authoring skills. Populate ARCHITECTURE.md (Component Inventory, Data Model, API Surface, Integration Points, Code Map, Event Topology) per the architecture skill's **Table Filling** rules — every applicable table must be populated. Populate the owning module's `specs/modules/{module}/INTERFACE.md` element and type rows and `DATA.md` tables from the code just read, per the module-authoring skill — extraction is exactly what these files exist to record. Generate all IDs in one batch call. Add newly discovered actors to ACTORS.md and newly discovered tech-stack entries to TECH-STACK.md per the reverse-engineering skill's project-level discovery rules.
...
- Record the target module set for the shared UC — Step 9 uses it to fan out the log entry with the `modules:` token.
...
## Step 8: Write the Cover Request

Create the change directory per the change-request skill (`date -u +%Y%m%dT%H%M%S` for the timestamp, short kebab-case slug) and write its `request.md`: the list of implementation files to spec and test, grouped per module and UC, each row linking the UC it now belongs to. Include the Non-canonical Test Paths list from Step 7, and close each group with the `### Additional Notes` and `### Examples` subsections — present even when empty. This is the run's scope: `molcajete build {change-id}` plans `test`-kind tasks from it.

## Step 9: Append Log Entry and Set UC Status

For every UC-XXXX touched in Step 7 (new or modified), iterate over every module-instance emitted: append the changelog entry per the `uc-log` shared skill, then write that instance's and its parent feature's status per the `status-rollup` shared skill. Extracted UC instances are always `pending` — they become `implemented` when a run builds their tests.
...
`specs/modules/auth/INTERFACE.md` gained 2 element rows; `DATA.md` gained 1 table.
...
Recorded in the cover request. The run reads each one as input for the canonical integration test and leaves the original in place.
...
Convention layout.** List every such path — `/m:cover` never moves or rewrites them. The cover request
records the list, and the run reads each file as input for the canonical integration test, leaving the
original in place.

End the report with the explicit hand-off:

> Next: run `molcajete build {change-id}` to cover the extracted UCs with integration tests (tests only, no production code).
