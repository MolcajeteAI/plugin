...
4. Extract the parent `FEAT-XXXX` from the directory path (under `features/`) and the module from the path (under `prd/modules/`).
...
- `prd/modules/{module}/features/FEAT-XXXX-{slug}/use-cases/UC-XXXX-{slug}.md` -- the use case containing this scenario
...
1. Edit `prd/modules/{module}/features/FEAT-XXXX-{slug}/use-cases/UC-XXXX-{slug}.md` -- modify only the `### SC-XXXX:` block with confirmed changes.
